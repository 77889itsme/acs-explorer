...
def acsexplorer_pipeline_by_location(addresses, geography, variables, year_range, dataset):
    """
...
def acsexplorer_pipeline_by_keyword(keyword, geography, year_range, dataset, top_search):
    """
...
def query_census_data(variable, start_year, dataset):
    """
    Query Census API for a specific variable and dataset.

    Parameters:
        variable (str): The variable name (e.g., "B28002_001E").
        start_year (int): The year to query data for (e.g., 2023).
        dataset (str): The dataset name (e.g., "acs1" or "acs5").

    Returns:
        str: The HTML content of the query response or error message.
    """
    import json
    base_url = f"https://api.census.gov/data/{start_year}/acs/{dataset}/variables/{variable}.json"
    try:
        response = requests.get(base_url)
        if response.status_code == 200:
            return json.loads(response.text)
        else:
            return f"Error: Unable to fetch data for {variable} ({response.status_code})."
    except Exception as e:
